...
if ( $img_hover ) {
    $wrapper_classes[] = athen_image_hover_classes( $img_hover );
}
if ( $style ) {
...
elseif ( $lightbox_video ) {
    ...
    if ( $lightbox_iframe_type ) {
        if ( 'video_embed' == $lightbox_iframe_type ) {
            $video_embed_url    = athen_sanitize_data( $lightbox_video, 'embed_url' );
...
if ( $img_filter ) {
    $css_class .= ' '. athen_image_filter_class( $img_filter );
}
