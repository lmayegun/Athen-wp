...
/**
 *
 * Description : Function for modifying how post excerpt are constructed. 
 * 
 * @package     Athen
 * @subpackage  Closer 
 * @since       Closer 1.0.0
 ...
 * 
 * Dependant : http://codex.wordpress.org/Function_Reference/wp_trim_words 
 */
 ...
 * @since Total 1.0.0
 */
function athen_excerpt( $args ) {
    echo athen_get_excerpt( $args );
}
 ...
 * @since 2.0.0
 */
function athen_get_excerpt( $args = array() ) {
    ...
    // Filter args
    $args = apply_filters( 'athen_excerpt_args', $args );
    ...
    if ( $post->post_password ) :
        ...
        $output = __( 'This is a password protected post.', 'athen_transl' );
        $output = apply_filters( 'athen_password_protected_excerpt', $output );
        $output = '<p>'. $output .'</p>';
        return $output;
    ...
    if ( $readmore ) :
        ...
        $read_more_text = isset( $args['read_more_text'] ) ? $args['read_more_text'] : __( 'Read more', 'athen_transl' );
    ...
    $output = apply_filters( 'athen_excerpt_output', $output );
 ...
 * @since Total 1.0.0
 */
function athen_excerpt_length() {

    // Theme panel length setting
    $length = athen_get_mod( 'blog_excerpt_length', '40' );
    ...
    if ( is_category() ) {
        ...
        if ( ! empty( $term_data['athen_term_excerpt_length'] ) ) {
            $length = $term_data['athen_term_excerpt_length'];
        }
    }

    // Return length and add filter for quicker child theme editign
    return apply_filters( 'athen_excerpt_length', $length );
 ...
 * @since Total 1.0.0
 */
function athen_excerpt_more( $more ) {
    return '&hellip;';
}
add_filter( 'excerpt_more', 'athen_excerpt_more', 10 );
 ...
 * @since Total 1.0.0
 */
function athen_custom_excerpt_length( $length ) {
    return '40';
}
add_filter( 'excerpt_length', 'athen_custom_excerpt_length', 999 );
 ...
 * @since Total 1.0.0
 */
function athen_remove_more_link_scroll( $link ) {
    $link = preg_replace( '|#more-[0-9]+|', '', $link );
    return $link;
}
add_filter( 'the_content_more_link', 'athen_remove_more_link_scroll' );