...
if ( ! class_exists( 'WPEX_Custom_WP_Gallery' ) ) {
    class WPEX_Custom_WP_Gallery {
        ...
        public function __construct() {
            ...
            // Add image sizes
            add_filter( 'athen_image_sizes', array( &$this, 'add_image_sizes' ), 999 );
        ...
        public function add_image_sizes( $sizes ) {
            $new_sizes  = array(
                'gallery'   => array(
                    'label'     => __( 'WordPress Gallery', 'athen_transl' ),
        ...
        function output( $output, $attr ) {
            ...
            // Set cropping sizes
            if ( $columns > 1 ) {
                $img_width  = $img_width ? $img_width : athen_get_mod( 'gallery_image_width' );
                $img_height = $img_height ? $img_height : athen_get_mod( 'gallery_image_height' );
            }

            // Sanitize Data
            $size   = ( ! $size && '1' == $columns ) ? 'large' : 'medium';
            $size   = ( $img_width || $img_height ) ? 'athen_custom' : $size;
            ...
            $output .= '<div id="gallery-'. $instance .'" class="wpex-gallery wpex-row lightbox-group clr">';
                ...
                foreach ( $attachments as $attachment_id => $attachment ) {
                    ...
                    // Attachment Vars
                    $attachment_data    = athen_get_attachment_data( $attachment_id );
                    $alt                = $attachment_data['alt'];
                    $caption            = $attachment_data['caption'];
                    $video              = $attachment_data['video'];
                    $lightbox_url       = $video ? $video : athen_get_lightbox_image( $attachment_id );

                    // Generate the image HTMl
                    $img_html = athen_get_post_thumbnail( array(
                    ...
                    if ( ! $video ) {
                        ...
                        $lightbox_data = ' data-options="thumbnail: \''. athen_get_lightbox_image( $attachment_id ) .'\', width:1920, height:1080"';
                    }
            
                    // Start Gallery Item
                    $output .= '<figure class="gallery-item '. athen_grid_class( $columns ) .' col col-'. $count .'">';
