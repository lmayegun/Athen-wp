...
if ( ! function_exists( 'vcex_portfolio_grid_shortcode_vc_map' ) ) {
	function vcex_portfolio_grid_shortcode_vc_map() {
		...
		// Add VC params
		vc_map( array(
			'name'                  => __( 'Portfolio Grid', 'athen_transl' ),
			'description'           => __( 'Recent portfolio posts grid', 'athen_transl' ),
			...
			'params'                => array(
				...
				array(
					...
					'description'   => __( 'Adds equal heights for the entry content so "boxes" on the same row are the same height. You must have equal sized images for this to work efficiently.', 'athen_transl' ),
				),
				array(
					'type'          => 'dropdown',
