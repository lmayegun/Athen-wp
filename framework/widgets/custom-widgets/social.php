<?php
/**
 * Description  : Class for generating social icon image buttons icons widget.
 * 
 * @package     Athen
...
// Start class
if ( ! class_exists( 'Athen_Social_Widget' ) ) {
    class Athen_Social_Widget extends WP_Widget {
        ...
        function __construct() {
            parent::__construct(
                'athen_social_widget',
                ATHEN_NAME_THEME . ' - '. __( 'Image Icons Social Widget', 'athen_transl' ),
                array(
                    'description'   => __( 'Displays icons with links to your social profiles with drag and drop support.', 'athen_transl' )
                )
            );
        }
        ...
        function widget( $args, $instance ) {
            ...
            if ( $social_services ) : ?>
                ...
                <?php
                // Define image location for social icons
                $img_location   = get_template_directory_uri() .'/images/social/';
                $img_location   = apply_filters( 'athen_social_widget_img_dir', $img_location ); ?>

                <ul class="athen-social-widget-img">
                    <?php foreach( $social_services as $key => $service ) { ?>
                        <?php $link = ! empty( $service['url'] ) ? $service['url'] : null; ?>
                        <?php $name = $service['name']; ?>
                        <?php if ( $link ) { ?>
                            <?php echo '<li><a href="'. esc_url( $link ) .'" title="'. esc_attr( $name ) .'"'. $target .'><img src="'. $img_location . strtolower ( $name ) .'.png" alt="'. esc_attr( $name ) .'" height="'. $size .'" width="'. $size .'" /></a></li>'; ?>
                        <?php } ?>
                    <?php } ?>
        ...
        function form( $instance ) {
            ...
            <p>
                <label for="<?php echo $this->get_field_id('size'); ?>"><?php _e('Size:', 'athen_transl'); ?></label>
                ...
                <small><?php _e('Size in pixels. Icon images are 36px.', 'athen_transl'); ?></small>
            </p>
            ...
            <small style="display:block;margin-bottom:10px;"><?php _e( 'Enter the full URL to your social profile', 'athen_transl' ); ?></small>
...
if ( ! function_exists( 'register_athen_social_widget' ) ) {
    function register_athen_social_widget() {
        register_widget( 'Athen_Social_Widget' );
    }
}
add_action( 'widgets_init', 'register_athen_social_widget' );
...
function load_athen_social_widget_scripts() {
    if ( !  is_admin() ) {
        return;
    }
    global $pagenow;
    if ( $pagenow == "widgets.php" ) {
        ...
        add_action( 'admin_footer', 'add_new_athen_social_ajax_trigger' );
        function add_new_athen_social_ajax_trigger() { ?>
            <script type="text/javascript" >
                jQuery(document).ready(function($) {
                    jQuery(document).ajaxSuccess(function(e, xhr, settings) { //fires when widget saved
                        var widget_id_base = 'athen_social_widget';
...
add_action( 'admin_init', 'load_athen_social_widget_scripts' );